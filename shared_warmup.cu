...
//catches error in cuda calls receives file and line 
...
int main(){
    ...
    T* d_in, *d_out;
    checkCuda(cudaMalloc(&d_in, N * sizeof(T)));
    checkCuda(cudaMalloc(&d_out, N * sizeof(T)));
    checkCuda(cudaMemcpy(d_in, h_in.data(), N * sizeof(T), cudaMemcpyHostToDevice));
