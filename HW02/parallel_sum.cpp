...
int main()
{
    ...
    for (int i = 0; i < T; ++i)
    {
        size_t startIndex = i * chunk;
        size_t endIndex = (i == T - 1) ? N : startIndex + chunk;
        threads.emplace_back(partial_sum, std::cref(data), startIndex, endIndex, std::ref(partials[i]));
    }
